...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (loggedIn) {
    ...
    const favoriteMovies = currentMovies.filter(movie => favorites.includes(movie.id));
...
function openMovieDetails(movieId) {
  ...
  const genreTags = movie.genre ? movie.genre.split(", ").map(g => `<span class="genre-tag">${g}</span>`).join("") : "";
