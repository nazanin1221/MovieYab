...
function setupEventListeners() {
  ...
  if (showFavoritesBtn) showFavoritesBtn.addEventListener("click", toggleFavoritesView);
  if (closeMovieModal) closeMovieModal.addEventListener("click", closeMovieDetails);
  ...
  modalContent.innerHTML = `
    <div class="flex flex-col md:flex-row gap-6">
      ...
      <div class="md:w-3/5">
        ...
        ${movie.genre ? `<div class="mb-4">${genreTags}</div>` : ""}
        <div class="plot-box">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="space-y-3">
            ${movie.director ? `<div class="info-item"><span class="info-label">Director:</span><span class="info-value">${movie.director}</span></div>` : ""}
            ${movie.writer ? `<div class="info-item"><span class="info-label">Writer:</span><span class="info-value">${movie.writer}</span></div>` : ""}
            ${movie.actors ? `<div class="info-item"><span class="info-label">Cast:</span><span class="info-value">${movie.actors}</span></div>` : ""}
          </div>
          <div class="space-y-3">
            ${movie.language ? `<div class="info-item"><span class="info-label">Language:</span><span class="info-value">${movie.language}</span></div>` : ""}
            ${movie.runtime ? `<div class="info-item"><span class="info-label">Runtime:</span><span class="info-value">${movie.runtime}</span></div>` : ""}
            ${movie.awards && movie.awards !== "Nothing" ? `<div class="info-item"><span class="info-label">Awards:</span><span class="info-value">${movie.awards}</span></div>` : ""}
            ${movie.released ? `<div class="info-item"><span class="info-label">Released:</span><span class="info-value">${movie.released}</span></div>` : ""}
          </div>
        </div>
...
document.querySelectorAll("button").forEach(btn => {
  if (btn.textContent.trim() === "Login") {
    btn.addEventListener("click", () => {
      ...
      setTimeout(() => loginModal.querySelector(".modalContent").classList.add("show"), 10);
    });
  }
});

closeModalBtn.addEventListener("click", () => {
  loginModal.querySelector(".modalContent").classList.remove("show");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  cardFavorites.addEventListener("click", () => {
    localStorage.setItem("showFavoritesOnLoad", "true");
    window.location.href = "search.html";
  });

  cardInfo.addEventListener("click", () => {
    alert("Explore detailed movie information in the search page!");
  });
});
